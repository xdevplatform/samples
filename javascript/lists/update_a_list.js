// Update the metadata of an existing list the authenticated user owns - using PIN-based OAuth to authorize the user
...
async function getRequest({ oauth_token, oauth_token_secret }) {
  ...
  const req = await got.put(endpointURL, {
    json: data,
    responseType: "json",
    headers: {
      Authorization: authHeader["Authorization"],
      "user-agent": "v2updateListJS",
